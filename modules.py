...
class ConvReluNorm(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size, n_layers, p_dropout):
        super().__init__()
        assert n_layers > 1, "n_layers should be larger than 0."
        self.n_layers = n_layers

        self.conv_layers = nn.ModuleList([nn.Conv1d(in_channels, hidden_channels, kernel_size, padding=kernel_size//2)])
...
class WN(torch.nn.Module):
    def __init__(
        ...
        assert(kernel_size % 2 == 1), f"Wavenet kernel_size must be odd integer, but got ({kernel_size})"
...
class ResBlock1(torch.nn.Module):
    ...
    def forward(self, x, x_mask=None):
        for c1, c2 in zip(self.convs1, self.convs2):
            # 在残差模块中，先进行激活操作是比较常见的。这是因为激活操作可以保留重要的特征并抑制噪声。
...
class Flip(nn.Module):
    def forward(self, x, *args, reverse=False, **kwargs):
        x = torch.flip(x, [1])  # 维度1: index => n-index, 翻转
        if not reverse:
...
class ElementwiseAffine(nn.Module):
    r"""
        forward: $y = m + e^{ln(s)} * x$
        
        reverse: $(y - m) * e^{-ln(s)} = x$
    """
...
class ResidualCouplingLayer(nn.Module):
    def __init__(
        ...
        n_layers, p_dropout=0, embed_dim=192, mean_only=False
    ):
        assert channels % 2 == 0, "input channels should be divisible by 2"
        super().__init__()
        ...
        self.enc = WN(hidden_channels, kernel_size, dilation_rate,
                      n_layers, p_dropout=p_dropout, embed_dim=embed_dim)
    ...
    def forward(self, x, x_mask, embed, reverse=False):
        ...
        h = self.enc(h, x_mask, embed=embed)
...
class ConvFlow(nn.Module):
    ...
    def forward(self, x, x_mask, embed, reverse=False):
        x0, x1 = torch.split(x, [self.half_channels]*2, 1)
        h = self.pre(x0)
        h = self.convs(h, x_mask, embed=embed)
